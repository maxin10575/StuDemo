...
public class ZkBackup {
	...
	public static void main(String[] args) throws IOException {
		...
		String zkdatapath = prop.getProperty("ZKDATAPATH");
		String usrname = prop.getProperty("USRNAME");
		String usrpwd = prop.getProperty("USRPWD");

		// 备份主机信息
		String backupip = prop.getProperty("BACKUPIP");
		String backupport = prop.getProperty("BACKUPPORT");
		String backupdatapath = prop.getProperty("BACKUPDATAPATH");
		String backusrname = prop.getProperty("BACKUPUSRNAME");
		String backusrpwd = prop.getProperty("BACKUPUSRPWD");
		...
		try {
			...
			timeout = 1200000;
			System.out.println("超时参数配置错误！！！使用默认值" + timeout);
		}
...
//		String cmdscpsnapshot = "scp -r " + snapshotfile + " " + backupip + ":" + backupdatapath+zkdatadir;
		String cmdcpsnapshot = "cp " + snapshotfile + " " + zkdatapath+"/"+zkdatadir;

		String logfile = logfilepath.trim();
//		String cmdscplog = "scp -r " + logfile + " " + backupip + ":" + backupdatapath+zkdatadir;
		String cmdcplog = "cp " + logfile + " " + zkdatapath+"/"+zkdatadir;
		
		String cmdfiledirtar = "tar -cvf "+ zkdatapath+"/"+zkdatadir+".tar.gz " + zkdatapath+"/"+zkdatadir;
		
		String cmdscp = " scp -r "+zkdatapath+"/"+zkdatadir+".tar.gz " + backupip +":"+backupdatapath;
