...
/**
 * @program: KafkaTest
 * @description: 2.11_2.4
 * @author: maxin
 * @create: 2020-03-11 12:51
 * @Modified By:
 * @Version: 1.0
 **/
...
//            Producer<String, String> producer = new KafkaProducer<String,String>(props);
       ...
       /* ListenableFuture<SendResult<String, Object>> future =
        ...
        for (int i = 0; i < 2; i++) {
            ...
            inmap.put("interests", "ball");
            inmap.put("address", "newwork");
            sendmap.put("name", "haha");
            sendmap.put("age", 10);
            sendmap.put("other", inmap);
            String mapAsString = sendmap.keySet().stream()
                    .map(key1 -> key1 + "=" + sendmap.get(key1))
                    .collect(Collectors.joining(", ", "{", "}"));
            System.out.println("sendMessage===" + mapAsString);
//                producer.send(new ProducerRecord<String, String>(TOPIC,key,mapAsString));
            producer.send(new ProducerRecord<>(TOPIC, mapAsString));
