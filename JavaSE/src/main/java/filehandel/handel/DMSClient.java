package filehandel.handel;

import filehandel.bo.LogData;
import filehandel.bo.LogRec;
...
/**
 * 该客户端运行在给用户提供unix服务的服务器上。 用来读取并收集该服务器上用户的上下线信息，并 进行配对整理后发送给服务端汇总。
 *
...
public class DMSClient {
     ...
     * 构造方法初始化第一步，解析配置文件
     *
     * @return 返回的Map中保存的是配置文件中的 每一条内容，其中key:标签的名字， value为标签中间的文本
     * @throws Exception
     */
    private Map<String, String> loadConfig() throws Exception {
        try {
            ...
            /*
             * 获取<config>标签中的所有子标签 并将每一个子标签的名字作为key,中间的 文本作为value存入Map集合
             */
    ...
    /**
     * 客户端开始工作的方法 循环执行三步: 1:解析日志 2:配对日志 3:发送日志
     */
    public void start() {
        parseLogs();
        matchLogs();
    ...
    private boolean sendLogs() {
        /*
         * 实现思路: 将logRecFile文件中的所有配对日志读取 出来然后连接上服务端并发送过去，若服务端
         * 全部接收，就可以将该文件删除，表示发送 完毕了。 实现步骤: 1:logRecFile文件必须存在
         * 2:将所有配对日志读取出来并存入一个集合 等待发送 3:通过Socket连接服务端 4:创建输出流 5:顺序将所有配对日志按行发送给服务端
         * 6:单独发送一个字符串"OVER"表示所有日志 均已发送完毕 7:创建输入流 8:读取服务端发送回来的响应字符串
         * 9:若响应的字符串为"OK",表示服务端正常 接收了所有日志,这时就可以将logRecFile 文件删除并返回true表示发送完毕。
         *
         */
        Socket socket = null;
        try {
     ...
     * 第二步:配对日志
     *
     * @return true:配对成功 false:配对失败
     */
    private boolean matchLogs() {

        /*
         * 实现思路: 将第一步解析的新日志，与上次为配对成功 的登入日志全部读取出来，然后再按照user,
         * pid相同，type一个是7，一个是8进行配对。 只要能找到类型为8的，一定可以找到一个 能与之配对的登入日志。
         *
         * 实现步骤: 1:必要的判断 1.1:logRecFile是否存在，存在则不再 进行新的配对工作，避免覆盖。
         * 1.2:textLogFile文件必须存在。 2:读取textLogFile将日志读取出来，并 存入到集合中。(若干LogData实例)
         * 3:若loginLogFile文件若存在，则说明 有上次未配对成功的日志，也将其读取 出来存入集合等待一起配对 4:配对工作
         * 4.1:创建一个集合，用于保存所有配对日志 4.2:创建两个Map分别保存登入日志与登出日志 4.3:遍历所有待配对的日志，按照登入与登出
         * 分别存入两个Map中， 其中key:user,pid value:LogData实例 4.4:遍历登出Map,并根据每条登出日志的key
         * 去登入Map中找到对应的登入日志，并 以一个LogRec实例保存该配对日志，然后 存入配对日志的集合中。并将该配对日志
         * 中的登入日志从登入Map中删除。这样一来 登入Map中应当只剩下没有配对的了。 5:将配对日志写入到logRecFile中
         * 6:将所有未配对日志写入到loginLogFile中 7:将textLogFile文件删除 8:返回true,表示配对完毕
         *
         */
        try {
            // 1
    ...
    private boolean parseLogs() {
        /*
         * 实现思路: 循环读取batch条日志，然后将每条日志中的 5个信息解析出来，最终组成一个字符串，以
         * 行为单位，写入到textLogFile文件中
         *
         * 实现步骤： 1:必要的判断工作 1.1:为了避免解析的日志还没有被使用，而 第一步又重复执行导致之前日志被覆盖
         * 的问题，这里需要判断，若保存解析后 的日志文件存在，则第一步不再执行。 该日志文件会在第二步配对完毕后删除。
         * 1.2:logFile文件必须存在(wtmpx文件) 1.3:是否还有日志可以解析
         * 2:创建RandomAccessFile来读取logFile 3:将指针移动到上次最后读取的位置，准备 开始新的解析工作 4:解析工作
         * 4.1:创建一个List集合，用于保存解析后 的每一条日志(LogData实例) 4.2:循环batch次，解析每条日志中的
         * 5项内容(user,pid,type,time,host) 并用一个LogData实例保存，然后将 该LogData实例存入集合
         * 5:将集合中的所有的日志以行为单位保存到 textLogFile中 6:保存书签信息 7:返回true,表示工作完毕
         *
         */
    ...
    /**
     * 第一步解析日志中的一个环节， 根据书签文件记录的位置判断是否还有 日志可以解析，若有，则将上次最后的位置 返回，若没有则返回-1。
     *
     * @return
     */
